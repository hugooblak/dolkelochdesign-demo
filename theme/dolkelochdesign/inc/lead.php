 ...
 * or photo. They call do_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Dolk El & Design AB.
 *
 ...
 * @return string
 */
function do_sample_note() {
	return do_lead( 'ar_riktig' ) ? '' : ' <span class="do-sample">(platshållare)</span>';
}
 ...
 * @return string File name for do_lead_img().
 */
function do_lead_bild( $i ) {
	$bilder = (array) do_lead( 'bilder', array() );
 ...
 * @return string
 */
function do_ring_label() {
	$namn = do_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, do_lead( 'telefon' ) ) : 'Ring ' . do_lead( 'telefon' );
}
