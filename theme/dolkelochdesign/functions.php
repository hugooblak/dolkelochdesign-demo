<?php
/**
 * Dolk El & Design AB theme functions.
 *
 * Kept short on purpose. Design lives in theme.json, markup in templates and patterns.
 * Lead capture lives in the "Dolk El & Design AB Leads" plugin, so leads survive a theme change.
 *
 ...
 * Patterns read these through do_lead() so one demo can be rebuilt for any firm.
 */
...
add_action(
	'wp_enqueue_scripts',
	function () {
		wp_enqueue_style( 'dolkelochdesign', get_stylesheet_uri(), array(), wp_get_theme()->get( 'Version' ) );
	}
);
add_action(
	'after_setup_theme',
...
add_action(
	'init',
	function () {
		register_block_style( 'core/paragraph', array( 'name' => 'eyebrow', 'label' => __( 'Eyebrow label', 'dolkelochdesign' ) ) );
		register_block_style( 'core/list', array( 'name' => 'checklist', 'label' => __( 'Tick list', 'dolkelochdesign' ) ) );

		register_block_pattern_category( 'dolkelochdesign-sections', array( 'label' => __( 'Dolk El & Design AB sections', 'dolkelochdesign' ) ) );
		register_block_pattern_category( 'dolkelochdesign-pages', array( 'label' => __( 'Dolk El & Design AB page layouts', 'dolkelochdesign' ) ) );
	}
);
