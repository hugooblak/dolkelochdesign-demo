...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'dolkelochdesign-leads' ),
					'singular_name' => __( 'Lead', 'dolkelochdesign-leads' ),
					'edit_item'     => __( 'Förfrågan', 'dolkelochdesign-leads' ),
					'search_items'  => __( 'Search leads', 'dolkelochdesign-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'dolkelochdesign-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'dolkelochdesign-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'dolkelochdesign-leads' ),
			'npl_phone'    => __( 'Phone', 'dolkelochdesign-leads' ),
			'npl_service'  => __( 'Needs', 'dolkelochdesign-leads' ),
			'npl_timeline' => __( 'When', 'dolkelochdesign-leads' ),
			'npl_zip'      => __( 'ZIP', 'dolkelochdesign-leads' ),
			'npl_source'   => __( 'Source', 'dolkelochdesign-leads' ),
			'npl_status'   => __( 'Status', 'dolkelochdesign-leads' ),
			'date'         => __( 'Received', 'dolkelochdesign-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'dolkelochdesign-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'dolkelochdesign-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'dolkelochdesign-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'dolkelochdesign-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'dolkelochdesign-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'dolkelochdesign-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'dolkelochdesign-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'dolkelochdesign-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'dolkelochdesign-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'dolkelochdesign-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'dolkelochdesign-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'dolkelochdesign-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
