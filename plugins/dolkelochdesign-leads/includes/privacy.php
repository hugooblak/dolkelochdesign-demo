...
add_filter(
	...
	function ( $exporters ) {
		$exporters['dolkelochdesign-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'dolkelochdesign-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'dolkelochdesign-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'dolkelochdesign-leads',
						'group_label' => __( 'Quote requests', 'dolkelochdesign-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['dolkelochdesign-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'dolkelochdesign-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
